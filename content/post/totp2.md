...
In my previous post ["SSH 2-Factor on Linux (RPi)"]({{< ref "/post/totp.md" >}}) we enable OTP as a second factor. That's a nice step in securing your SSH sessions however it's not the only,
and in fact is usually not the first step.
...
but it does prevent the rival from the club down the road from gaining entry just by overhearing the password, unless of course he steals the card from me.

The way we solve this is by taking extra precautions to protects those factors. By employing a combination of something we have and something we know, we add further protection
because it's difficult to compromise these things using the same method.

# What factors are there?
Digitally, authentication factors can come in many forms such as passwords, cryptographic keys, one-time-pass generators, Smart Cards, and many more

These commonly come in the form of a digital file, or string of characters. 
...
Other devices in the form of a Smart Card (such as a chip-and-pin bank card) or a USB device like the FIDO "dongles" or a Yubikey are also all forms that such devices can be found in.
...
and what kind of device would mitigate the risks.

* If a password is practical to remember, and you always have a phone on which you can install an app for TOTP (Time-based One-Time Pass), then go with that.
...
any calls and messages that were intended for you, such as a one-time-pass or other SMS or Phone based confirmation.
...
If you're using a Virtual Private Server in the cloud, make sure they have console access and you're able to access it.

> _If you lose your SSH private key you will no longer be able to SSH into your machine_ and will be forced to login by other means to install a new key.
...
During generation you'll most likely be prompted to give the private key a password to protect it, I highly recommend, in fact, I insist, set one.
...
> In this tutorial we'll use `RSA 2048`, because it is the default and generally well supported. Feel free to read up on alternatives such as `ECDSA` and `ED25519` and decide if you'd prefer those, or simply a longer RSA key.

#### 1.0 - Public & Private

**Whichever platform you use, the important thing to remember is that SSH keys come in pairs; a public and a private key.
...
> Note: This password does not need to match any other, it is used specifically to unlock this key.
...
> Fun Fact: The randomart image is a human-friendly way you can visually compare the fingerprints without reading long strings. It's not currently in wide use (or enabled by default).
...
The process of installing you Public Key however is not complicated. First I'll explain how to do it, then I'll explain what is happening for those interested.
...
Now you can try logging in with your new key, on Linux it's really simple, just login with username and server address
```shell
...
In this dialog you can also choose to unlock the key automatically on login.
...
Now we need to configure SSH properly on the Pi (or server) to ensure we're prompted for our OTP.
...
this is the key I "hamid" use to login from my "Desktop".
