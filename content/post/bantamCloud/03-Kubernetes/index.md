...
There a numerous ways to run Kubernetes, with a plethora of certified distributions, hosted options, installers and more, all [certified by the Cloud Native Computing Foundation (CNCF)](https://www.cncf.io/certification/software-conformance/). The CNCF is the place to look for all things in the Kubernetes ecosystem, but if you're here, you hopefully already have a good idea of what Kubernetes is all about.

What we need as a solution for deploying Kubernetes in a lean, simple to deploy way, bearing in mind we want to run on Raspberry Pi which uses the ARM architecture.
...
Choosing a distribution is really down to preference, so long as the distribution supports your desired architecture (ARM in this case).
...
There are other similar options such as [K0s](https://github.com/k0sproject/k0s), also written in Go and supporting both ARM and x86-64 architectures, I have no experience with this distribution.

The beauty of Kubernetes Certified distributions is that once you understand and build for one, you can mostly shift your deployments to another and run it there. The caveat being that many cloud distributions offer "add-on" functionality such as cloud load balancers, storage options etc, which are provider dependent, and usually more integrated than bare-metal options.

I have run code on K3s, Digital Ocean's DOKS, Amazon EKS (AWS), Minikube and "full-fat" Kubernetes deployed using Kubespray. On the whole, the bulk of the work is similar, with some key differences, covered in the previous part 2.5 of this series, "caveats". These differences are usually fairly minor, and can be handled with annotations, or a change in a few variables for a Helm Chart; The point is, learn Kubernetes with one, and you'll be in good standing to run on Kubernetes elsewhere.
...
By default, K3s will use [Traefik](https://github.com/traefik/traefik) as its Ingress Controller, and will use an internal service-load-balancer. This will not however provide load-balancing between your nodes from outside of the cluster, for this an external load balancer is required, or something like [MetalLB](https://metallb.universe.tf/). For Ingress Controllers, Traefik is fun and nice to use, but for serious workloads I prefer [Ingress NGINX](https://kubernetes.github.io/ingress-nginx/), not to be confused with [NGINX Ingress Controller](https://docs.nginx.com/nginx-ingress-controller/)

If you intend to use MetalLB and/or Traefik, export as required the following no-deploy arguments respectively, before installing K3s;
...
First, pick a control-plane node, previously known as a "master" node.

This is the node which will coordinate the cluster. In production deployments, this should be several nodes, for high availability, but for our little cluster, a single control-plane node will suffice.

SSH into your first node, the one you've chosen to be the control-plane and run the following command;

> NOTE: Given my security and privacy concious nature, I'm obligated to recommend you read the contents of the shell script this command is going to run on your machine before running it.

```
curl -sfL https://get.k3s.io | sh -
```

Once it has completed, you'll need to obtain the server token, this is used by your worker nodes to join the cluster. Obtain it by issuing the following command;

```
sudo cat /var/lib/rancher/k3s/server/token
```

Next, go to each of your remaining nodes in turn and run the following command, substituting the server token obtained in the previous step;
...
Copy the file at `/etc/rancher/k3s/k3s.yaml` on the control-plane node to your local machine, and edit the line `server: https://127.0.0.1:6443` changing the IP to point at the Pi's IP address rather than localhost.
...
Alternatively, call the file whatever you wish, palce it wherever you wish and export the environment variable `KUBECONFIG` with the path to the file.
...
> Ensure you have a subnet available on your router which can be allocated to the cluster, and that ARP is working correctly, as MetalLB will use Layer 2 / ARP to notify of IP address changes. The address range also needs to be correctly routable by your router. I suggest reading the Helm website for further details.

> If you have issues deploying MetalLB in cluster, it is always possible to deploy a load-balancer external to the cluster, see the previous section of this series for examples of external load-balancer options.
...
then create a file named `values.yaml`; for reference, the contents of my `values.yaml` are as follows;
...
If all went well, you should now be able to deploy a workload and MetalLB will assign an IP.

Let's deploy `8k` to test;
...
Once the system upgrade controller is deployed, some upgrade "Plans" are required, one for server and one for agents;

## Server Upgrade Plan
```
# First deploy the upgrade controller: https://github.com/rancher/system-upgrade-controller/releases/download/v0.8.0/system-upgrade-controller.yaml
...
The agent upgrade plan waits for the server upgrade job to complete so when it is deployed isn't important, just deploy both plans and wait.
...
Once the jobs have been applied, check the status. Note, it will take a few minutes, but eventually both Plans should run on each node;
